...
export class SkinningAnimation extends CanvasAnimation {
  ...
  /* Highlighted bone rendering info */
  private highlightRenderPass: RenderPass;

  /* Cyan cylinder wireframe for the hovered bone */
  private cylinderRenderPass: RenderPass;
  private lastCylinderBone: number = -2; // force first build
  ...
  public getScene(): CLoader { return this.scene; }
  ...
  public initModel(): void {
    ...
    for (let i = 0; i < faceCount * 3; i += 3) {
      fIndices[i] = i; fIndices[i + 1] = i + 1; fIndices[i + 2] = i + 2;
    }
    ...
    this.sceneRenderPass.setDrawData(this.ctx.TRIANGLES,
      this.scene.meshes[0].geometry.position.count, this.ctx.UNSIGNED_INT, 0);
  ...
  public initSkeleton(): void {
    ...
    this.skeletonRenderPass.addUniform("uHighlightBone",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
  ...
  public initHighlight(): void {
    ...
    this.highlightRenderPass.addAttribute("boneIndex", 1, this.ctx.FLOAT, false,
    ...
    this.highlightRenderPass.addUniform("mWorld",
      (gl, loc) => { gl.uniformMatrix4fv(loc, false, new Float32Array(Mat4.identity.all())); });
    this.highlightRenderPass.addUniform("mProj",
      (gl, loc) => { gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all())); });
    this.highlightRenderPass.addUniform("mView",
      (gl, loc) => { gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all())); });
    this.highlightRenderPass.addUniform("bTrans",
      (gl, loc) => { gl.uniform3fv(loc, this.getScene().meshes[0].getBoneTranslations()); });
    this.highlightRenderPass.addUniform("bRots",
      (gl, loc) => { gl.uniform4fv(loc, this.getScene().meshes[0].getBoneRotations()); });
  ...
  public initCylinderWireframe(): void {
    ...
    this.cylinderRenderPass.addAttribute("vertPosition", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, new Float32Array(6));
  ...
  public initFloor(): void {
    ...
    this.floorRenderPass.addAttribute("aVertPos", 4, this.ctx.FLOAT, false,
      4 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, this.floor.positionsFlat());
    ...
    this.floorRenderPass.setDrawData(this.ctx.TRIANGLES,
      this.floor.indicesFlat().length, this.ctx.UNSIGNED_INT, 0);
      ...
      if (hi >= 0) {
        ...
        if (hi !== this.lastCylinderBone) {
          ...
          if (indices.length > 0) {
            ...
            this.cylinderRenderPass.addAttribute("vertPosition", 3, gl.FLOAT, false,
              3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, positions);
            this.cylinderRenderPass.addUniform("mView",
              (g, loc) => { g.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all())); });
            this.cylinderRenderPass.addUniform("mProj",
              (g, loc) => { g.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all())); });
