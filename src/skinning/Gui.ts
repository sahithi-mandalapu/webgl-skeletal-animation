...
export class GUI implements IGUI {
  private static readonly rotationSpeed: number = 0.004;
  private static readonly zoomSpeed: number     = 0.1;
  private static readonly rollSpeed: number     = 0.08;
  private static readonly panSpeed: number      = 0.01;
  private static readonly boneRadius: number    = 0.01;
  ...
  private rotatingBone: boolean    = false;
  private translatingBone: boolean = false;
  ...
  private dragAxisH: Vec3 | null = null;
  private dragAxisV: Vec3 | null = null;
  ...
  private boneLocalRotAtDragStart: Quat | null = null;
  ...
  public setCamera(
    pos: Vec3, target: Vec3, upDir: Vec3,
  ...
  private screenToRay(screenX: number, screenY: number): { origin: Vec3; dir: Vec3 } {
  ...
  private rayCylinder(origin: Vec3, dir: Vec3, p: Vec3, q: Vec3, radius: number): number {
    const axis    = Vec3.difference(q, p);
    const axisLen = axis.length();
    if (axisLen < 1e-6) return Infinity;
    ...
    for (const t of [(-b - sqrtDisc) / (2 * a), (-b + sqrtDisc) / (2 * a)]) {
      ...
      if (proj >= 0 && proj <= axisLen) return t;
    }
    return Infinity;
  }
  ...
  public pickBone(screenX: number, screenY: number): number {
    const yInViewport = this.height - screenY - this.viewPortOffsetY;
    if (yInViewport < 0 || yInViewport > this.viewPortHeight) return -1;
  ...
  public getCylinderWireframe(boneIndex: number, segments: number = 8): {
    ...
    if (Math.abs(Vec3.dot(axis, perp)) > 0.9) perp = new Vec3([0, 1, 0]);
    const u = Vec3.cross(axis, perp); u.normalize();
    const v = Vec3.cross(axis, u);    v.normalize();
    ...
    for (let ring = 0; ring < 2; ring++) {
      ...
      for (let s = 0; s < segments; s++) {
        ...
        positions.push(
          center.x + r * (Math.cos(angle) * u.x + Math.sin(angle) * v.x),
          center.y + r * (Math.cos(angle) * u.y + Math.sin(angle) * v.y),
          center.z + r * (Math.cos(angle) * u.z + Math.sin(angle) * v.z),
        );
      }
    }
      ...
      if (this.rotatingBone && this.selectedBone >= 0
        ...
        if (meshes.length > 0) {
          ...
          if (bone.parent < 0) {
            ...
            bone.rotation = Quat.product(
              meshes[0].bones[bone.parent].rotation,
              bone.localRotation
            );
          }
  ...
  public onKeydown(key: KeyboardEvent): void {
    switch (key.code) {
      case "Digit1": { this.animation.setScene("./static/assets/skinning/split_cube.dae");  break; }
      case "Digit2": { this.animation.setScene("./static/assets/skinning/long_cubes.dae");  break; }
      case "Digit3": { this.animation.setScene("./static/assets/skinning/simple_art.dae");  break; }
      case "Digit4": { this.animation.setScene("./static/assets/skinning/mapped_cube.dae"); break; }
      case "Digit5": { this.animation.setScene("./static/assets/skinning/robot.dae");       break; }
      case "Digit6": { this.animation.setScene("./static/assets/skinning/head.dae");        break; }
      case "Digit7": { this.animation.setScene("./static/assets/skinning/wolf.dae");        break; }

      case "KeyW": { this.camera.offset(this.camera.forward().negate(), GUI.zoomSpeed, true); break; }
      case "KeyA": { this.camera.offset(this.camera.right().negate(),   GUI.zoomSpeed, true); break; }
      case "KeyS": { this.camera.offset(this.camera.forward(),          GUI.zoomSpeed, true); break; }
      case "KeyD": { this.camera.offset(this.camera.right(),            GUI.zoomSpeed, true); break; }
      ...
      case "KeyK": {
        if (this.mode === Mode.edit) {
          const meshes = this.animation.getScene().meshes;
          if (meshes.length > 0) meshes[0].addKeyframe();
        }
        break;
      }
  ...
  private rollBone(boneIndex: number, angle: number): void {
    const meshes = this.animation.getScene().meshes;
